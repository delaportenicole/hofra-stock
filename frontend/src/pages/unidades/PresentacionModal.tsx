...
import type { Presentacion, CreatePresentacionDto, UpdatePresentacionDto } from '@hofra/shared';
...
type FormData = CreatePresentacionDto & { activo?: boolean };

export function PresentacionModal({ isOpen, onClose, onSuccess, presentacion }: Props) {
  ...
  }, [isOpen, presentacion, reset]);

  const onSubmit = async (data: FormData) => {
    try {
      if (isEditing) {
        await presentacionesService.update(presentacion.id, data as UpdatePresentacionDto);
        toast.success('Presentación actualizada');
      } else {
        await presentacionesService.create(data);
